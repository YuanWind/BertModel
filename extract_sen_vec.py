...
sess = tf.Session(config=config)  #注意 ，这里为tensorflow2.0版本，与第1.0有差距。ktf_keras.set_session (sess)
...
def gen_sen_vec(sen):
    pooled = bert_vector.bert_encode_sen([sen])
    vec = pooled[0]
    vec = np.array (vec)
    vec.tolist ()
    return vec
def gen_words_vec(sen):
    pooled = bert_vector.bert_encode_word ([sen])
    return pooled
